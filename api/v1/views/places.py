#!/usr/bin/python3
"""The module implement a rule that return a view"""
...
def place_by_city(city_id):
    """to view function that return place objects by city"""
...
def delete_place(place_id):
    """The endpoint that delete a Place object"""
    place = storage.get(Place, place_id)
    if place is None:
        abort(404)
    place.delete()
...
def insert_place(city_id):
    """The endpoint that insert a Place object"""
    city = storage.get(City, city_id)
    if city is None:
        abort(404)
...
def places_search():
    """It retrieves all Place objects depending of the body of the request"""
...
def update_place(place_id):
    """Its endpoint that update a Place object"""
    place = storage.get(Place, place_id)
    if place is None:
        abort(404)
    res = request.get_json()
